...
export const FIRST_TAB_URL = 'broxme://wellcome/'
export const DEFAULT_URL = 'broxme://wellcome/'
...
export function create (opts) {
  ...
  var page = {
    ...
    isGuessingTheURLScheme: false, // did broxme guess at the url scheme? if so, a bad load may deserve a second try
    ...
    siteInfoOverride: null, // explicit overrides on the siteinfo, used by broxme: pages
    ...
    getIntendedURL () {
      ...
      if (url.startsWith('broxme:') && page.siteInfoOverride && page.siteInfoOverride.url) {
        // override, only if on a builtin broxme site
...
function onDidStopLoading (e) {
  ...
  if (page) {
    ...
    var { protocol, hostname, pathname, version } = url.startsWith('http://') ? parseDatURL(url) : parseURL(url)
    page.siteInfo = null
    page.sitePerms = null
...
function onDidFailLoad (e) {
  ...
  if (page) {
    ...
    // if https fails for some specific reasons, and *assumed* https, then fallback to http
...
async function updateHistory (page) {
  ...
  if (!url.startsWith('broxme://') || url.match(/broxme:\/\/library\/[0-9,a-f]{64}/g)) {
