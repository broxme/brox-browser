...
export class PageMenuNavbarBtn {
  ...
  async onClickEditSettings () {
    this.close()
    pages.setActive(pages.create('broxme://settings'))
  }
  ...
  //   page.loadURL(datUrl || 'broxme://start/')
  // }
  ...
  onClickViewFiles () {
    ...
    pages.setActive(pages.create(`broxme://library/dat://${page.siteInfo.key}`))
  }
  ...
  onClickNetworkDebugger () {
    ...
    const page = pages.getActive()
    if (!page || !page.getViewedDatOrigin()) return
    pages.setActive(pages.create(`broxme://swarm-debugger/dat://${page.siteInfo.key}`))
  }
  ...
  async onClickFork () {
    ...
    if (fork) {
      page.loadURL(`broxme://library/${fork.url}#setup`)
    }
  }
