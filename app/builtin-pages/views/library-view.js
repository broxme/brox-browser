...
async function setup () {
  ...
  try {
    ...
    if (archive.url.startsWith(archive.info.url) === false) {
      window.location = 'broxme://library/' + archive.info.url
      return
    }
...
async function gotoFileEditor (filePath) {
  // go to the new path
  window.history.pushState('', {}, `broxme://library/${archive.url}/${filePath}`)
  await readViewStateFromUrl()
  onOpenFileEditor()
}
...
function renderVersionPicker () {
  ...
  // is the version a number?
  if (version == +version) {
    link = yo`<a href="broxme://library/${archive.checkout('latest').url}">View latest</a>`
    version = `v${version}`
  } else if (syncPath) {
      ...
      total
        ? yo`
          <a
            ...
            href=${`broxme://library/${archive.url}#local-compare`}
...
function renderSettingsView () {
  ...
  const baseUrl = `broxme://library/${archive.url}`
  const currentFaviconUrl = `beaker-favicon:32,${archive.url}?cache=${faviconCacheBuster}`
        ...
        <div class="section">
          ...
          <div class="section-content">
            ...
            ${isOwner
              ? yo`
                  <p>
                    Enter a link to your donation page and Brox will show
                    a <span class="fa fa-donate"></span> icon in your page's URL bar.

                    ${renderSettingsField({key: 'paymentLink', value: paymentLink, placeholder: 'Example: https://opencollective.com/broxbrowser', onUpdate: setManifestValue})}
                  </p>
                `
              : paymentLink
    ...
    <div class="container">
                ...
                ${isSaved
                  ? [
                    ...
                    <div class="hint">
                      ...
                      <a href="https://support.broxme.com" target="_blank" class="learn-more-link">Learn more</a>
                    </div>`
                  }
              </div>
            </div>
      ...
      <div class="view network">
        <div class="section">
          ...
          <a href="broxme://swarm-debugger/${archive.key}" target="_blank">
...
function renderToolbar () {
  ...
  return yo`
    <div class="library-toolbar">
      <div class="container">
        <a href="broxme://library" class="back-link">
...
async function onMakeCopy () {
  ...
  window.location = `broxme://library/${fork.url}#setup`
}
...
async function onDeletePermanently () {
  ...
  if (confirm(`Delete ${nickname} permanently?`)) {
    try {
      ...
      window.location = 'broxme://library'
    } catch (e) {
      console.error(e)
...
async function onSetCurrentSource (node) {
  ...
  window.history.pushState('', {}, `broxme://library/${path}`)
}
...
async function onArchiveUpdated (e) {
  if (e.details.url === archive.checkout().url) {
    ...
    if (isOwner && (isSavedChanged || previewModeChanged)) {
      ...
      window.location = `broxme://library/${archive.checkout().url}`
      return
    }
    await archive.setup()
  ...
  if (e.details.url === archive.url) {
    ...
    noticeBanner.create('error', yo`
      <div>
        ...
        Brox detected a cyclical symlink at <code>${cyclePath}</code> and had to abort.
      </div>`
    )
    render()
  }
...
function markdownHrefMassager (href) {
  ...
  if (isRelative) {
    ...
    return `broxme://library/${archive.url}/${href}`
  }
  return href
}
