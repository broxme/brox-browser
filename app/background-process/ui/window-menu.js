    ...
    submenu: [
      {
        ...
        accelerator: 'Command+,',
        click (item, win) {
          if (win) viewManager.create(win, 'broxme://settings', {setActive: true})
      ...
      { type: 'separator' },
      { label: 'Hide Brox', accelerator: 'Command+H', role: 'hide' },
      ...
      { type: 'separator' },
          {
            ...
            enabled: !noWindows,
            click: function (item, win) {
              if (win) viewManager.create(win, 'broxme://internal-archives/', {setActive: true})
            }
          }, {
            label: 'Open Dat-DNS Cache Page',
            enabled: !noWindows,
            click: function (item, win) {
              if (win) viewManager.create(win, 'broxme://dat-dns-cache/', {setActive: true})
            }
          }, {
            label: 'Open Debug Log Page',
            enabled: !noWindows,
            click: function (item, win) {
              if (win) viewManager.create(win, 'broxme://debug-log/', {setActive: true})
            }
          }]
      },
      {
  ...
  var historyMenu = {
    ...
    submenu: [
      ...
      },
      {
        ...
        click: function (item, win) {
          if (win) viewManager.create(win, 'broxme://history', {setActive: true})
    ...
    submenu: [
      {
        ...
        accelerator: 'F1',
        click: function (item, win) {
          if (win) viewManager.create(win, 'https://support.broxme.com/', {setActive: true})
        }
      },
      {
        label: 'Report Bug',
        click: function (item, win) {
          if (win) viewManager.create(win, 'https://github.com/broxme/brox/issues', {setActive: true})
        }
      }
    ]
  }
