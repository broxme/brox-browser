...
const Y_POSITION = 78
const DEFAULT_URL = 'broxme://wellcome'
...
class View {
  ...
  async updateHistory () {
    ...
    if (!/^broxme:\/\/(start|history)/i.test(url)) {
...
export function openOrFocusDownloadsPage (win) {
  win = getTopWindow(win)
  var views = getAll(win)
  var downloadsView = views.find(v => v.url.startsWith('broxme://downloads'))
  if (!downloadsView) {
    downloadsView = create(win, 'broxme://downloads')
  }
